#!/usr/bin/env python3
"""Regenerate 015_surat_types.sql seed with escaped single quotes."""
import json
...
def format_jsonb_array(arr):
    ...
    for item in arr:
        if isinstance(item, str):
            # Handle each character class separately to avoid double-escaping.
            # The JSON stores literal \n as two chars: chr(92)='\' + chr(110)='n'.
            # In PostgreSQL single-quoted strings, \n = newline (actual char 10).
            # PostgreSQL sees \\n = two chars: \ and n (escaped backslash + literal n).
            # So we escape \ → \\ for PostgreSQL. For JS expressions, ' → \' as well.
            result = []
            for ch in item:
                o = ord(ch)
                if o == 92:   # backslash
                    result.append("\\\\")
                elif o == 39: # single quote (inside JS template expressions)
                    result.append("\\'")
                elif o == 34: # double quote
                    result.append('\\"')
                elif o == 10: # actual newline → PostgreSQL escape
                    result.append("\\n")
...
print("Done:", MIGRATION_PATH)
print(len(inserts), "entries |", len(seed_sql), "chars")
