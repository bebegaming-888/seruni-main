-- ============================================================
-- COMPLETE LETTER LAYOUT MIGRATION
-- File: supabase/migrations/FINAL_057_letter_layouts.sql
-- Run in Supabase SQL Editor: https://supabase.com/dashboard/project/wrfraskmawmciiutwcpx/sql/new
-- ============================================================

-- 1. Create handle_updated_at function (required by all tables)
...
create policy "Anyone can view active signers"
  on public.letter_signers for select using (is_active = true);
create policy "Admins manage signers"
  on public.letter_signers for all using (auth.role() = 'authenticated');
...
create policy "Anyone can view active rejection reasons"
  on public.rejection_reasons for select using (is_active = true);
create policy "Admins manage rejection reasons"
  on public.rejection_reasons for all using (auth.role() = 'authenticated');
...
-- 4. letter_layouts table
create table if not exists public.letter_layouts (
  id                  uuid primary key default uuid_generate_v4(),
  surat_type_code     text not null,
  name                text not null,
  description         text,
...
create unique index if not exists letter_layouts_active_unique
  on public.letter_layouts(surat_type_code) where status = 'active';
create index if not exists letter_layouts_surat_type_idx on public.letter_layouts(surat_type_code);
create index if not exists letter_layouts_status_idx on public.letter_layouts(status);

alter table public.letter_layouts enable row level security;
create policy "Anyone can view layouts"
  on public.letter_layouts for select using (true);
create policy "Authenticated manage layouts"
  on public.letter_layouts for insert with check (auth.role() = 'authenticated');
create policy "Authenticated update layouts"
  on public.letter_layouts for update using (auth.role() = 'authenticated');
create policy "Authenticated delete layouts"
  on public.letter_layouts for delete using (auth.role() = 'authenticated');

-- 5. letter_layout_history table
...
create index if not exists letter_layout_history_layout_idx on public.letter_layout_history(layout_id);

alter table public.letter_layout_history enable row level security;
create policy "Anyone can view layout history"
  on public.letter_layout_history for select using (true);
create policy "Authenticated insert history"
  on public.letter_layout_history for insert with check (auth.role() = 'authenticated');

-- 6. Helper functions
create or replace function public.get_active_layout(p_surat_type_code text)
returns public.letter_layouts language plpgsql security definer as $$
begin
  return (select * from public.letter_layouts where surat_type_code = p_surat_type_code and status = 'active' limit 1);
end;
$$;

create or replace function public.list_active_signers()
returns setof public.letter_signers language plpgsql security definer as $$
begin
  return query select * from public.letter_signers where is_active = true order by position_order;
end;
$$;

create or replace function public.list_active_rejection_reasons()
returns setof public.rejection_reasons language plpgsql security definer as $$
begin
  return query select * from public.rejection_reasons where is_active = true order by position_order;
end;
$$;
...
do $$
begin
  raise notice '✅ Migration 057 complete: letter_layouts, letter_signers, rejection_reasons, letter_layout_history tables created';
end;
$$;