...
**Target:** Mengurangi antrian di kantor desa, mempercepat waktu pemrosesan surat, dan mengeliminasi proses manual berbasis kertas (*paperless office*).
...
**Relevansi Mobile-First:**
Panel admin dirancang *responsive* — perangkat desa (Kades, Operator, Verifikator) dapat memproses surat langsung dari smartphone saat di luar kantor.
...
- [x] Sync layer `useSupabaseSync.ts` menjembatani localStorage ↔ Supabase (write-through)
...
- [ ] Endpoint verifikasi surat publik (berdasarkan nomor surat)
...
Edge Fns:    Cloudflare Pages Functions (send-wa, generate-pdf)
Sync Layer:  src/lib/useSupabaseSync.ts (write-through localStorage → Supabase)
...
*Dokumen ini adalah pedoman aktif. Setiap selesai menyelesikan task, perbarui checklist yang relevan di dokumen ini.*